...
def get_audio_files(categories):
    category_names = [category[0] for category in categories]
    chosen_files = []
    num_files_to_choose = [3, 2, 1] if len(category_names) > 1 else [6]
...
def merge_selected_files(input_files):
    ...
    for infile_path in input_files:
        ...
        non_silence = detect_nonsilent(audio, min_silence_len=1000, silence_thresh=-50)
    ...
    combined.export("userselection//output//output.wav", format="wav")
